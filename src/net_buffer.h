...
struct Raw_data {
    ...
    constexpr Raw_data (const uint8_t* pointer, unsigned int size) : pointer{pointer}, size_{size} {}
    constexpr Raw_data() = default;
    constexpr const size_t   size()  const { return size_; }
    constexpr const uint8_t* begin() const { return pointer; }
    constexpr const uint8_t* end()   const { return pointer + size_; }
};
