...
{
	cmsis.CR1 = 0;
	adc.set(decltype(adc)::Resolution::_6bits);
	BOOST_CHECK_EQUAL (cmsis.CR1, ADC_CR1_RES_Msk);

   cmsis.CR1 = 0;
	adc.set(decltype(adc)::Resolution::_8bits);
   BOOST_CHECK_EQUAL (cmsis.CR1, ADC_CR1_RES_1);

   cmsis.CR1 = 0;
	adc.set(decltype(adc)::Resolution::_10bits);
   BOOST_CHECK_EQUAL (cmsis.CR1, ADC_CR1_RES_0);

   cmsis.CR1 = 0;
	adc.set(decltype(adc)::Resolution::_12bits);
...
BOOST_AUTO_TEST_CASE (Sample_time)
{
   ...
   cmsis.SMPR1 = 0;
	...
	// cmsis.SMPR1 = 0; test::static_assert_exception:
	// adc.set<19>(decltype(adc)::Sample_time::_3CLK);
	// BOOST_CHECK_EQUAL (cmsis.SMPR1, 0);
}
...
BOOST_AUTO_TEST_CASE (set_regular_sequence_order)
{
	...
	// cmsis.SQR1 = 0; // test::static_assert_exception:
	// adc.set_regular_sequence_order<0, 8>();
	// BOOST_CHECK_EQUAL (cmsis.SQR1, ADC_SQR1_SQ14_3);

	// cmsis.SQR1 = 0; // test::static_assert_exception:
	// adc.set_regular_sequence_order<14, 19>();
	// BOOST_CHECK_EQUAL (cmsis.SQR1, ADC_SQR1_SQ14_3);
}
...
BOOST_AUTO_TEST_CASE (set_regular_sequence_length)
{
	...
	// cmsis.SQR1 = 0; // test::static_assert_exception:
	// adc.set_regular_sequence_length<0>();
	// BOOST_CHECK_EQUAL (cmsis.SQR1, 0);

	// cmsis.SQR1 = 0; // test::static_assert_exception:
	// adc.set_regular_sequence_length<17>();
	// BOOST_CHECK_EQUAL (cmsis.SQR1, 0);
}
